...
<div class="max-w-2xl mx-auto py-10 px-6">
    ...
    <div class="bg-white shadow-lg rounded-xl p-8">
        ...
        <form action="{{ route('exams.mock-exams.store', $exam) }}" method="POST">
            @csrf
            <!-- 模試名 -->
            <div class="mb-6">
                <label class="block text-sm font-medium mb-2">模試名</label>
                <input type="text" name="name" value="{{ old('name') }}" class="w-full border-gray-300 rounded-lg">
            </div>
            <!-- 受験日 -->
            <div class="mb-6">
                <label class="block text-sm font-medium mb-2">受験日</label>
                <input type="date" name="taken_at" value="{{ old('taken_at') }}" class="w-full border-gray-300 rounded-lg">
            </div>
            <!-- 偏差値 -->
            <div class="mb-6">
                <label class="block text-sm font-medium mb-2">偏差値（任意）</label>
                <input type="number" name="deviation_value" value="{{ old('deviation_value') }}" step="0.1" min="0" max="100" class="w-full border-gray-300 rounded-lg" placeholder="例: 58.5">
            </div>
            <!-- 科目スコア -->
            <div class="mb-6">
                ...
                <div id="subjects-container">
                    ...
                    @foreach ($oldSubjects as $index => $subject)
                        <div class="subject-row flex gap-2 mb-2">
                            <input type="text" name="subjects[]" value="{{ $subject }}" placeholder="科目" class="w-1/2 border-gray-300 rounded-lg">
                            <input type="number" name="scores[]" value="{{ $oldScores[$index] ?? '' }}" placeholder="点数" class="w-1/2 border-gray-300 rounded-lg">
